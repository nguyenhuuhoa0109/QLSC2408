...
import { HomeTreeNode, createDefaultHomeTree } from './home/homeTreeData';
import { HomeTreeView } from './home/HomeTreeView';
...
export const OverviewScreen: React.FC<OverviewScreenProps> = ({
  ...
  // Home Screen Tree View State
  const [homeTree, setHomeTree] = useState<HomeTreeNode>(() => createDefaultHomeTree(plantName));
  const [selectedHomeNode, setSelectedHomeNode] = useState<HomeTreeNode>(() => {
    const defaultTree = createDefaultHomeTree(plantName);
    return defaultTree.children?.[0] || defaultTree;
  });
  const [showTreeSidebar, setShowTreeSidebar] = useState<boolean>(true);

  // Update tree if plantName changes
  React.useEffect(() => {
    setHomeTree(prev => ({
      ...prev,
  ...
  const handleSelectHomeNode = (node: HomeTreeNode) => {
    setSelectedHomeNode(node);
    if (node.type === 'domain') {
      setSelectedDomain(node.domain || 'all');
    } else if (node.type === 'warehouse-dashboard' || node.type === 'warehouse-category') {
      setSelectedDomain('warehouse');
    } else if (node.type === 'maintenance-status') {
      setSelectedDomain('maintenance');
    } else if (node.type === 'doc-category') {
      setSelectedDomain('document');
  ...
  const handleAddCustomFolder = (parentId: string, folderName: string) => {
    const addRecursively = (node: HomeTreeNode): HomeTreeNode => {
      if (node.id === parentId) {
        const newChild: HomeTreeNode = {
          id: `custom-home-${Date.now()}`,
          name: folderName,
          type: 'custom',
          isCustom: true
        };
    ...
    setHomeTree(prev => addRecursively(prev));
  };

  // Compute breadcrumbs path
  const breadcrumbs = useMemo(() => {
    const path: HomeTreeNode[] = [];
    const findPath = (current: HomeTreeNode): boolean => {
      path.push(current);
      if (current.id === selectedHomeNode.id) return true;
      if (current.children) {
  ...
  return (
    <div className="flex flex-col w-full gap-5 sm:gap-6 p-4 sm:p-6 bg-[#f9f9ff]">
      ...
      {/* Top Breadcrumb & Tree View Toggle Bar */}
      <div className="bg-white p-3.5 rounded-2xl border border-[#e2eaf5] shadow-xs flex flex-wrap items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          >
            <FolderTree size={14} />
            <span>{showTreeSidebar ? 'Ẩn Cây thư mục' : 'Hiện Cây thư mục'}</span>
          </button>
          ...
          {breadcrumbs.map((crumb, idx) => {
            ...
            return (
              <React.Fragment key={crumb.id}>
                ...
                <button
                  onClick={() => handleSelectHomeNode(crumb)}
                  className={`font-semibold cursor-pointer transition-colors ${
                    isLast 
                      ? 'text-[#005394] font-bold bg-[#eef3fb] px-2 py-0.5 rounded-md' 
                      : 'text-gray-600 hover:text-[#005394]'
                  }`}
                >
                  {crumb.name}
                </button>
        ...
        <div className="flex items-center gap-2">
          {selectedHomeNode.type === 'warehouse-dashboard' ? (
            <button
              onClick={() => {
                const allNode = homeTree.children?.[0] || homeTree;
                handleSelectHomeNode(allNode);
              }}
            ...
            >
              <Layers size={13} />
              <span>Xem Tổng quan Trang chủ</span>
            </button>
          ) : selectedHomeNode.type !== 'all' ? (
            <button
              onClick={() => {
                const allNode = homeTree.children?.[0] || homeTree;
                handleSelectHomeNode(allNode);
            ...
            >
              <Layers size={13} />
              <span>Xem Toàn bộ hệ thống</span>
            </button>
          ) : (
            <button
              onClick={() => {
                const dashNode = homeTree.children
                  ?.find(c => c.type === 'domain' && c.domain === 'warehouse')
                  ?.children?.find(c => c.type === 'warehouse-dashboard');
                if (dashNode) handleSelectHomeNode(dashNode);
              }}
              className="px-2.5 py-1 bg-[#005394]/10 hover:bg-[#005394]/20 text-[#005394] font-bold rounded-lg text-xs transition-colors cursor-pointer flex items-center gap-1"
            >
              <LayoutDashboard size={13} />
              <span>Mở Dashboard Kho (Thư mục con)</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 items-start w-full">
        ...
        {showTreeSidebar && (
          <div className="w-full lg:w-72 xl:w-80 flex-shrink-0">
            <HomeTreeView
              treeData={homeTree}
              selectedNodeId={selectedHomeNode.id}
              inventory={inventory}
              maintenanceTasks={maintenanceTasks}
              documents={documents}
              onSelectNode={handleSelectHomeNode}
        ...
        <div className="flex-1 min-w-0 w-full flex flex-col gap-5">
          {/* Case 1: Warehouse Dashboard selected as a subfolder */}
          {selectedHomeNode.type === 'warehouse-dashboard' ? (
            <WarehouseDashboardView
              inventory={inventory}
              subView={selectedHomeNode.dashboardSubView}
              onSelectCategoryFolder={(categoryName) => {
                const catNode = homeTree.children
                  ?.find(c => c.type === 'domain' && c.domain === 'warehouse')
                  ?.children?.find(c => c.id === 'node-home-wh-all')
                  ?.children?.find(c => c.categoryFilter === categoryName);
                if (catNode) {
                  handleSelectHomeNode(catNode);
                } else {
                  onNavigateToWarehouse();
                }
              }}
              onOpenNewTransaction={onOpenNewTransaction}
              onViewAllItems={onNavigateToWarehouse}
            />
          ) : selectedHomeNode.type === 'warehouse-category' ? (
            /* Case 2: Specific Warehouse Category folder selected */
            <div className="flex flex-col gap-4">
              <div className="bg-white p-5 rounded-2xl border border-[#e2eaf5] shadow-xs flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#e7eeff] text-[#005394] flex items-center justify-center">
                    <Package size={20} />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-base font-bold text-gray-900">
                        {selectedHomeNode.name}
                      </h2>
                      <span className="px-2 py-0.5 bg-[#eef3fb] text-[#005394] rounded-full text-xs font-bold">
                        {inventory.filter(i => i.category === selectedHomeNode.categoryFilter).length} vật tư
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Phân mục trực thuộc: <strong>Quản lý kho</strong> &gt; Danh mục lưu trữ
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-[#005394] hover:bg-[#004278] text-white text-xs font-bold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <Plus size={14} />
              ...
              <div className="bg-white rounded-2xl border border-[#e2eaf5] shadow-xs overflow-hidden">
                <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-gray-700">
                    Danh sách vật tư trong thư mục ({inventory.filter(i => i.category === selectedHomeNode.categoryFilter).length})
                  </h3>
                  <span className="text-xs text-gray-400">Đơn vị: chiếc, bộ, kg, lít...</span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-[#f8fafd] text-[#414750] border-b border-[#e2eaf5] uppercase text-[10px] font-bold">
                      <tr>
                        <th className="py-2.5 px-4">Mã</th>
                        <th className="py-2.5 px-4">Tên vật tư</th>
                        <th className="py-2.5 px-4">Tồn kho / Định mức</th>
                        <th className="py-2.5 px-4">Vị trí lưu kho</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {inventory
                        .filter(i => i.category === selectedHomeNode.categoryFilter)
                        .map(item => (
                          <tr key={item.id} className="hover:bg-blue-50/40 transition-colors">
                            <td className="py-3 px-4 font-mono font-bold text-[#005394]">{item.code}</td>
                            <td className="py-3 px-4 font-semibold text-gray-800">{item.name}</td>
                            <td className="py-3 px-4">
                              <span className="font-mono font-bold">{item.quantity}</span> / {item.minQuantity} {item.unit}
                            </td>
                            ...
                            <td className="py-3 px-4 text-center">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                                ...
                                {item.status === 'critical' ? 'Khẩn cấp' : item.status === 'warning' ? 'Cảnh báo tồn' : 'Đạt định mức'}
                              </span>
                            </td>
                            <td className="py-3 px-4 text-right">
                              <button
                                onClick={() => onOpenNewTransaction('import', item)}
                                className="px-2 py-1 bg-gray-100 hover:bg-[#005394] hover:text-white rounded text-[11px] font-semibold transition-colors cursor-pointer"
                              >
                                Lập phiếu
                              </button>
                            </td>
          ...
          ) : selectedHomeNode.type === 'maintenance-status' ? (
            /* Case 3: Maintenance Status folder selected */
            <div className="flex flex-col gap-4">
              <div className="bg-white p-5 rounded-2xl border border-[#e2eaf5] shadow-xs flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-gray-900">
                      Hạng mục sửa chữa: {selectedHomeNode.name}
                    </h2>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Trạng thái: <strong>{selectedHomeNode.statusFilter}</strong>
                    </p>
                  </div>
                </div>
                ...
                {onNavigateToMaintenance && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <Wrench size={14} />
                    <span>Mở màn hình Quản lý sửa chữa</span>
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {maintenanceTasks
                  .filter(t => t.status === selectedHomeNode.statusFilter)
                  .map(task => (
                    <div key={task.id} className="bg-white p-4 rounded-2xl border border-[#e2eaf5] shadow-xs flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-bold text-gray-900 text-sm">{task.title}</span>
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                            task.priority === 'Khẩn cấp' || task.priority === 'Cao' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'
                          }`}>
                            {task.priority}
                          </span>
                        </div>
                        ...
                        <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                          <span>Thiết bị: <strong>{task.equipment}</strong></span>
                          <span>Phụ trách: <strong>{task.assignedTo}</strong></span>
                        </div>
                      </div>
                      <div className="mt-3 pt-2 border-t border-gray-100 flex items-center justify-between text-xs">
                        <span className="text-gray-400">Tiến độ: {task.progressPercent}%</span>
                        <span className="font-mono text-gray-500">{task.estimatedCompletion}</span>
                      </div>
                    </div>
                  ))}
              </div>
            </div>
          ) : selectedHomeNode.type === 'doc-category' ? (
            /* Case 4: Document Category folder selected */
            <div className="flex flex-col gap-4">
              <div className="bg-white p-5 rounded-2xl border border-[#e2eaf5] shadow-xs flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-gray-900">
                      Tài liệu kỹ thuật: {selectedHomeNode.name}
                    </h2>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Danh mục: <strong>{selectedHomeNode.categoryFilter}</strong>
                    </p>
                  </div>
                </div>
                ...
                {onNavigateToDocuments && (
                  ...
                  >
                    <FileText size={14} />
                    <span>Mở màn hình Hồ sơ tài liệu</span>
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {documents
                  .filter(d => d.category === selectedHomeNode.categoryFilter)
                  .map(doc => (
                    <div key={doc.id} className="bg-white p-4 rounded-2xl border border-[#e2eaf5] shadow-xs flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                          <span>Mã: <strong className="font-mono">{doc.code}</strong></span>
                          <span>Tác giả: <strong>{doc.author}</strong></span>
                        </div>
                      </div>
                      <div className="mt-3 pt-2 border-t border-gray-100 flex items-center justify-between text-xs">
                        <span className="text-gray-400">{doc.fileSize} • {doc.downloadsCount} lượt tải</span>
                        <span className="text-[#005394] font-bold">v{doc.version}</span>
                      </div>
                    </div>
                  ))}
              </div>
            ...
            /* Case 5: Default All Overview / System Domain Dashboard */
            <>
